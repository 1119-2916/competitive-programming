...
Line get_bisection(const Point &p1, const Point &p2) {
    Circle c1(p1, 2000000), c2(p2, 2000000); //INF だと誤差が酷い
...
// 直線と直線の角度
double get_line_argle(Line &a, Line &b)
{
    if (a.p1.x > a.p2.x) swap(a.p1, a.p2);
    if (b.p1.x > b.p2.x) swap(b.p1, b.p2);
    Point s1 = Point(a.p2.x - a.p1.x, a.p2.y - a.p1.y),
          s2 = Point(b.p2.x - b.p1.x, b.p2.y - b.p1.y);
    return angle(Vector(s1), Vector(s2));
}
...
signed main()
{
    ...
    map<int, double> ans;
    Polygon funami = convex_hull_with_points_online(input);
    rep(i, funami.size()) {
        Point l = funami[(i-1 + funami.size()) % funami.size()],
            r = funami[(i+1 + funami.size()) % funami.size()];
        Line ll = get_bisection(l, funami[i]), 
             rl = get_bisection(r, funami[i]);
        std::cout << l << " " << funami[i] << " " << r << " : ";
        double angle = get_line_argle(ll, rl) / (PI * 2);
        std::cout << angle << std::endl;
        ans[index[funami[i]]] = angle;
    }

    rep(i, funami.size()) {
        std::cout << funami[i] << " " << ans[index[funami[i]]] << endl;
    }
    cout << "=====================" << endl;

    rep(i, n) {
        if (ans.find(i) != ans.end()) {
            std::cout << Decimal << ans[i] << std::endl;
        } else {
            std::cout << Decimal << 0.0 << std::endl;
        }
    }
