...
signed main()
{
    ...
    rep(i, n) {
        int p_inv = mod_inverse(n - i, MOD);
        int q_inv = (n - i - 1) * mod_inverse(n - i, MOD) % MOD;
        //std::cout << p_inv << " " << q_inv << std::endl;
        rep(j, x+1) {
            dp[i+1][j % data[i]] = 
                (dp[i+1][j % data[i]] + dp[i][j] * p_inv) % MOD;
            dp[i+1][j] = (dp[i+1][j] + dp[i][j] * q_inv) % MOD;
        }
    }
    ...
    */
    ...
    int ans = 0, shoe_n = 1;
    rep(i, n) shoe_n = (shoe_n * (i + 1)) % MOD;
    //std::cout << shoe_n << std::endl;
    rep(i, x+1) {
        ans = (ans + dp[data.size()][i] * ((shoe_n * i) % MOD)) % MOD;
    }
