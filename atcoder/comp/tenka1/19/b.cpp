...
signed main()
{
    ...
    rep(i, data.size()) {
        for (int j = dp.size()-1; j > 0; j--) {
            dp[j+data[i]] = (dp[j+data[i]] + dp[j]) % MOD;
            dp[j] = (dp[j] * 2ll) % MOD;
        }
        dp[data[i]] = (dp[data[i]] + dp[0]) % MOD;
        dp[0] = (dp[0] * 2ll) % MOD;
    }
    vi bq(90001, 0);
    bq[0] = 1;
    rep(i, data.size()) {
        for (int j = bq.size()-1; j > 0; j--) {
            bq[j+data[i]] = (bq[j+data[i]] + bq[j]) % MOD;
        }
        bq[data[i]] = (bq[data[i]] + bq[0]) % MOD;
    }

    int ans = repow(3, n, MOD);
    for (int i = ceil(sum/2.0); i < dp.size(); i++) {
        if (dp[i]) ans = (ans - ((dp[i] * 3ll) % MOD) + MOD) % MOD;
    }
    
    if (sum % 2) {
        std::cout << ans << std::endl;
    } else {
        std::cout << (ans + (bq[sum/2] * 3ll)) % MOD << std::endl;
    }
