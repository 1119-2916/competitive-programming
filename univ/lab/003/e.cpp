...
signed main()
{
    ...
    SegmentTree funami(n);
    for (int i = n-1; i >= 0; i--) {
        if (data[i] + i >= n) {
            funami.update(i, 1);
        } else {
            int yui = funami.query(i+1, i + data[i]+1);
            if (yui == data[i]) {
                funami.update(i, 0);
            } else {
                funami.update(i, 1);
    ...
    rep(i, n) {
        cout << funami.query(i, i+1) << " ";
    }
    cout << endl;
    if (funami.query(0, 1)) {
        cout << "OMAEGAGOTEDA!TAIKYOKUKAISHI!" << endl;
    } else {
        cout << "OMAEGASENTEDA!TAIKYOKUKAISHI!" << endl;
    }
