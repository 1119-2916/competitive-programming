...
signed main()
{
    ...
    sort(all(data));
    reverse(all(data));
    auto res = unique(all(data));
    data.erase(res, data.end());
    vvi table(data.size()+1, vi(pow(2, 17), 0));
    ...
    rep(i, data.size()) {
        rep(j, table[i].size()) {
            ...
            table[i+1][j] %= MOD;
            table[i+1][j ^ data[i]] += (table[i][j] * memo[data[i]]) % MOD;
            table[i+1][j] += (table[i][j] * memo[data[i]]) % MOD;
            table[i+1][j ^ data[i]] %= MOD;
        }
    }

    cout << table[data.size()][k] << endl;
