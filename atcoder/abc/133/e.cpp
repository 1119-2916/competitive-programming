...
int solve(Graph &g, int pos, int per)
{
    int ret = 1;
    if (per == -1) {
        int cnt = g[pos].size();
        if (k < cnt+1) return 0;
        int ret = (mod_comb(k, cnt+1, MOD) * fact[cnt+1]) % MOD;
        //cout << "-1 : " << ret << endl;
        for (auto i : g[pos]) {
            if (i.d != per) ret = (ret * solve(g, i.d, pos)) % MOD;
        }
        return ret;
    } else {
        int cnt = g[pos].size();
        if (k-2 < cnt-1) return 0;
        int ret = (mod_comb(k-2, cnt-1, MOD) * fact[cnt-1]) % MOD;
        for (auto i : g[pos]) {
            if (i.d != per) ret = (ret * solve(g, i.d, pos)) % MOD;
        }
        return ret;
