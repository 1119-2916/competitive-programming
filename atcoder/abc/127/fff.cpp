...
vp target(vvi &input)
{
    ...
    rep(i, q) {
        int tmp = input[i+1][0];
        if (tmp == 1) {
            int a = input[i+1][1], b = input[i+1][2];
            c += b;
            mm.insert(a);
            sum++;
            if (a < mid) {
            ...
            if (mid == INF) {
                ...
                int nmid = mm.rank((sum-1)/2LL)->key;
                //cout << "nmid = " << nmid << " ";
                if (nmid < mid) { // 小さい方に挿入
                    right += mid;
                    left -= nmid;
            ...
            if (mid == INF) {
                ...
                ret.pb({mid, c + (lc * mid) - left + right - (rc * mid)});
            }
        }
    }
    return ret;
